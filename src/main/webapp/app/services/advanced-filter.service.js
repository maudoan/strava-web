    ...
    function AdvancedFilter($translate, apiData, $http, $timeout,$sce,User,AlertService, ErrorHandle,TableMultiple, $scope) {
        ...
        function initAdvancedFilter(scope) {
            ...
            $scope.getRemoveFavorites = function (){
                return $http.get(HOST_GW + '/api/default-configs/get-remove-favorites?userId='+$scope.user.id +'&page=' + $scope.modelAdFilter).then(function(response) {
            ...
            $scope.getFavorites =  function  (){
                ...
                $scope.getRemoveFavorites().then(function (removes) {
                    ...
                    return $http.get(HOST_GW + '/api/personal-configs/search?' + params).then(function(response) {
            ...
            $scope.runFilter = function () {
                ...
                $http.post(HOST_GW + '/api/advanced-filters/parse',$scope.allFilters[$scope.modelAdFilter]).then(function (response) {
                    $scope.TABLES[$scope.tableAdFilter].param_current_page = 0;
            ...
            $scope.selectFavorite = function(index){
                if($scope.favorites[$scope.modelAdFilter][index].default){
                    ...
                    return $http.get(HOST_GW + '/api/personal-configs/set-default?id=' + $scope.favorites[$scope.modelAdFilter][index].id + '&userId=' + $scope.user.id +'&isDefault=true').then(function() {
                        $scope.loadFavoriteConfig();
                    });
                } else {
                    $scope.detachFavorite();
                    $scope.favoriteIconStype[$scope.modelAdFilter] = {};
                    return $http.get(HOST_GW + '/api/personal-configs/set-default?id=' + $scope.favorites[$scope.modelAdFilter][index].id + '&userId=' + $scope.user.id +'&isDefault=false').then(function() {
            ...
            function deleteFavorite(id){
                return $http.get(HOST_GW + '/api/personal-configs/remove?id='+ id +'&userId=' +$scope.user.id).then(function(response) {
                    return response;
                });
            };
        ...
        function sumQuantity (query,model,fieldQuery) {
            return $http.post(HOST_GW + '/api/advanced-filters/sum-quantity?'+query +"&model=" + model,fieldQuery).then(function(response) {
                return response.data;
            });
        }

        function createAdFilter(adFilter){
            return $http.post(HOST_GW + '/api/advanced-filters',adFilter).then(function(response) {
                return response.data;
            });
        }

        function deleteAdFilter(id){
            return $http.delete(HOST_GW + '/api/advanced-filters/'+id).then(function(response) {
                return response;
            });
        };
        ...
        function createFavorite(currentFavorite) {
            return $http.post(HOST_GW + '/api/personal-configs',currentFavorite).then(function(response) {
                return response.data;
            });
        };
