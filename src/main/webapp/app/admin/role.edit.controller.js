(function(){
    ...
    RoleEditController.$inject = ['$rootScope','$scope','$state','$stateParams','Role','Privilege','AlertService','AlertModalService', 'ErrorHandle', '$translate','variables', '$timeout', 'ComboBoxController'];
    function RoleEditController($rootScope,$scope, $state, $stateParams, Role, Privilege, AlertService, AlertModalService,ErrorHandle, $translate, variables, $timeout, ComboBoxController) {
        var vm = this;
        $scope.ComboBox = {};
        $scope.blockModal;
        ...
        $scope.submit = function (isClose) {
            ...
            var $form = $('#form_createrole');
            if (!$scope.form_createrole.$valid) return;
            ...
            if (tmp_priv.length > 0) {
                ...
                for (var i = 0; i < tmp_priv.length; i++) {
                    ...
                    for(var j = 0 ; j < $scope.allPrivileges.length; j++) {
                        if($scope.allPrivileges[j].categoryName == priv_i){
            ...
            if($scope.role.type == 1){
                AlertModalService.error("Vai trò mặc định, bạn không có quyền sửa");
                return;
            }
            $scope.blockUI();
            Role.update($scope.role).then(function (data) {
                if($scope.blockModal != null) $scope.blockModal.hide();
                AlertModalService.success("success.msg.update");

                $timeout(function () {
                    let path = location.origin;
                    isClose ? location.replace(path + '/#/roles') : location.replace(path + '/#/roles/' + data.id + '/detail');
                    location.reload();
                },1100);
            }).catch(function (data) {
                if($scope.blockModal != null) $scope.blockModal.hide();
