(function () {
    ...
    RoleCreateController.$inject = ['$rootScope', '$scope', '$state', 'Role', 'Privilege', 'AlertService', 'AlertModalService', '$translate', 'variables', '$timeout', 'ErrorHandle', 'ComboBoxController', 'User', '$window'];

    function RoleCreateController($rootScope, $scope, $state, Role, Privilege, AlertService, AlertModalService, $translate, variables, $timeout, ErrorHandle, ComboBoxController, User, $window) {
        ...
        $scope.submit = function (isClose) {
            ...
            Role.create($scope.role).then(function (data) {
                if($scope.blockModal != null) $scope.blockModal.hide();
                AlertModalService.popup("success.msg.create");
                $timeout(function () {
                    isClose ? $state.go('roles') : $state.go('roles-detail',{roleId: data.id});
                },1100);
            })
        ...
        if (angular.element('#form_createrole').length) {
            ...
            $scope.maxLength256 = $translate.instant("global.messages.maxLength256");
