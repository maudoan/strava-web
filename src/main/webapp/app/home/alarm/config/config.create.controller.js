(function () {
    ...
    angular.module('erpApp')
            .controller('AlarmConfigCreateController', AlarmConfigCreateController);

    AlarmConfigCreateController.$inject = ['$rootScope', '$scope', '$state', '$http', '$timeout',
        'AlertService', '$translate', 'TableController', 'ComboBoxController', 'AlertModalService', 'AlarmConfig', 'Device', 'ErrorHandle', '$window'];

    function AlarmConfigCreateController($rootScope, $scope, $state, $http, $timeout,
            AlertService, $translate, TableController, ComboBoxController, AlertModalService, AlarmConfig, Device, ErrorHandle, $window) {
        ...
        $scope.areaChange = function () {
            $timeout(function () {
                ...
                console.log($scope.gatewayOptions);
        ...
        // toan tu bang = thi khong gui ban tin xuong thiet bi
        ...
        $scope.gatewayChange = function () {
            $timeout(function () {
                if ($scope.alarmConfig.gatewayId) {
                    ...
                    console.log($scope.selectedCbb.gateway[0]);
                } else {
        ...
        if (angular.element('#form_create_alarm_config').length) {
            var $formValidate = $('#form_create_alarm_config');
        ...
        $scope.submit = function (isClose) {
            ...
            $('#form_create_alarm_config').parsley();
            var $form = $('#form_create_alarm_config');
            if (!$scope.form_create_alarm_config.$valid) {
                return;
            }
            ...
            $scope.blockUI();
            getParameters($scope.alarmConfig.areaShortName, $scope.alarmConfig.unit);
            createAlarmConfig(isClose);
            //$scope.btnDisable = true;
        };

        $scope.checkNameExist = null;
        function checkName(alarmName) {
            var param = "query=name=='" + alarmName.trim() + "'";
            AlarmConfig.getPageFull(param).then(function (response) {
            ...
            return $scope.checkNameExist;
        }

        function createAlarmConfig(isClose) {
            if ($scope.checkboxEmailOnsite == false) {
                ...
                checkName($scope.alarmConfig.name);
                var interval_obj = setInterval(function () {
                    if ($scope.checkNameExist != null) {
                        if ($scope.checkNameExist === true) {
                            AlertModalService.handleOneError("Tên cảnh báo đã tồn tại");
                            $scope.btnDisable = false;
                        } else {
                            AlarmConfig.create($scope.alarmConfig).then(function (data) {
                                if ($scope.blockModal != null) {
                                    $scope.blockModal.hide();
                                }
                                AlertModalService.popup("success.msg.create");
                                $timeout(function () {
                                    isClose ? $state.go('alarm-config') : $state.go('alarm-config-detail', {configId: data.id}); //redirect sang page khac
                                }, 1100);
                            }).catch(function (data) {
                                if ($scope.blockModal != null) {
                                    $scope.blockModal.hide();
