(function(){
    ...
    ConfigController.$inject = ['$rootScope','$scope','$state','$translate',
        '$http', 'ErrorHandle', '$window', 'TableController', 'Config', 'AlertService', 'ComboBoxController', '$timeout', 'HOST_GW'];
    function ConfigController($rootScope,$scope, $state, $translate,
                           $http, ErrorHandle, $window, TableController, Config, AlertService, ComboBoxController, $timeout, HOST_GW) {
        $scope.ComboBox = {};
        $scope.log = {};
        ...
        $scope.defaultDelete = function () {
            TableController.defaultDelete(tableConfig.tableId, Config.deleteMany);
        };
    }
})();